...
color: <?=$data['secondary_font_color']?> !important;
}
...
html *{
font-weight: 400;
<?php if (trim($data['primary_font_family'])!=''): ?>
color: <?=$data['primary_font_family']?>;
<?php else: ?>
...
#calipio-container > span {
display: flex;
justify-content: center;
align-items: center;
}

.round-recording-icon{
...
<h2 class='refokus-logo primary-font-color primary-font-family'><?=$data['name_of_business']?></h2>
...
<label for="name" class='input-sub-text FBVerify'>or verify with facebook
...
<div id="calipio-container">
<span><div class='round-recording-icon'></div>click to record</span>
<calipio-recorder id="calipio-recorder" class="calipio-recorder" style="display: block; width: fit-content; margin: 0 auto;" mandatorysources="camera microphone" selectedsources="camera microphone" allowedsources="camera microphone" startmode="immediate" endmode="immediate" hidepopupwhile="during-setup after-recording" token="eyJhcGlfdG9rZW4iOiJmMzZlMWI3ZC0yYzUwLTQyNWMtYTA3Ni1mYjg3Yjg3MDYyNjMiLCJwdWJsaWNfa2V5IjoiWjVaRGhPbU5XVnVha3UvSDdGaUp4V0J3S3FMVmF1UWVHSmppNndUeXdIcz0ifQ==" onrecordingended="calipioRecEnd(event);" onrecordingcountdownstarted="calipioRecStart(event);">
...
<label for="email" class='reward-notice'>
<?=$data['reward']?>
</label>
<div class="text-input-group">
...
<p class="copyright"><?=$data['footer_text']?></p>
</span>
</form>
...
<h2 class='refokus-logo primary-font-color primary-font-family'><?=$data['name_of_business']?></h2>
<div class="leftside-title-container">
<h1 class="title primary-font-color primary-font-family">Will You Do Us a Favor?</h1>
<h3 class="secondary-font-color secondary-font-family desc">
...
<div>
<a id="share-twitter" class="twitter-share-button <?= isset($reviewData['share_twitter']) && $reviewData['share_twitter'] == 1 ? '':'hidden' ?>" target="_blank">
...
function updateCalipioUI(){
const calipioContainer = document.getElementById("calipio-recorder")
const host = calipioContainer.getElementsByTagName("span")[0]
...
function showCalipioUI(){
document.getElementById("calipio-recorder").style.opacity = "1"
}

function hideCalipioUI(){
document.getElementById("calipio-recorder").style.opacity = "0"
}
...
$('input[name="name"]').siblings('.input-group-addon ').addClass('verified');
$('input[name="name"]').closest('.form-group').addClass('has-success');
if(response.email) {
$('input[name="email"]').val(response.email).attr('readonly', true);
$('input[name="email"]').siblings('.input-group-addon').addClass('verified email');
$('input[name="email"]').closest('.form-group').addClass('has-success');
}
var h = '<div class="fbVerified">';
if(response.picture.data.url) {
h += '<img class ="fbimage" src="' + response.picture.data.url + '"/>';
}
