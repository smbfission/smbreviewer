...
if (isset($_REQUEST['submit']) && isset($_REQUEST['short_io_api_key'])) {
  ...
  $sql = "UPDATE `user` set  `short_io_api_key` = case when `advanced_settings`= 1 then  '" . $_REQUEST['short_io_api_key'] . "' else NULL end where `id` = '" . $_SESSION['user_id'] . "'";
  mysqli_query($conn, $sql);
  $_SESSION['msg'] = "Short.io API Key is saved";
} else if (isset($_REQUEST['reset']) && $_REQUEST['reset'] == 'reset') {
  $_SESSION['msg_type'] = 'info';
  $sql = "UPDATE `user` SET `short_io_api_key` = NULL where `id` = '" . $_SESSION['user_id'] . "'";
  mysqli_query($conn, $sql);
  $_SESSION['msg'] = "Short.io API Key is restored to default";
}
          ...
          <form action="" method="post" class="form-horizontal form-bordered">
            <?php if ((int)$row['advanced_settings'] == 0) : ?>
              ...
              <div class="form-group">
                <label class="col-md-3 control-label" for="inputDefault">Short.io API Key: </label>
                <div class="col-md-6">
                  <input class="form-control" name="short_io_api_key" id="short_io_api_key" value="<?php echo @$row['short_io_api_key']; ?>" type="text" placeholder="<?= ((@$row['use_default_credentials'] == 1) ? "default value" : "Enter Short.io API key") ?>" />
                </div>
              </div>
