...
if (isset($_REQUEST) && isset($_REQUEST['action'])) {
    ...
    if ($_REQUEST['action'] == "plans") {
        ...
        if (isset($_REQUEST['template_code']) && $_REQUEST['template_code'] != "") {
            unlink('./uploads/templates/' . $currentTemplate['template']);
        ...
        if (isset($_FILES["preview-image"]) && $_FILES["preview-image"]['size'] > 0) {
            $target_dir = './uploads/templates/previews/';
            $rand = $_REQUEST['template_name'] . '_' . time();
        ...
        $result = $db->insertReviewTemplate($_REQUEST['template_name'], $template, $preview_image);
        ...
        if ($result) {
            unlink('./uploads/templates/' . $template['template']);
            unlink('./uploads/templates/previews/' . $template['preview_image']);
        }
        ...
        $preview_image = '';
        if (isset($_FILES["preview-image"]) && $_FILES["preview-image"]['size'] > 0) {
            unlink('./uploads/templates/previews/' . $currentTemplate['preview_image']);
